...
class UartLogging(threading.Thread):
    ...
    def handle_exception(self,e):
        if((e.__class__.__name__ == "OSError" and re.search('resource\s*temporarily\s*unavailable',e.strerror,re.I)) or e.__class__.__name__ == "SerialException"):
            if(not self.uart_issue):
                self.uart_issue = True
                self.log_exception_traceback("*** Serial Port seems to be compromised . UART logs might miss out prints... ***")
            return
        else:
    ...
    def compile_parser_regexes(self):
        self.re_partition_updation = re.compile("\s*end\s*updating\s*(\w*)|\s*update partition\s*(.*)\s*success",re.IGNORECASE)
        ...
        self.re_bootloader_version = re.compile(".*\[bootloader\]\s*\(version(.*)\)|.*cboot\s*version\:*\s*(.*)",re.IGNORECASE)
    ...
    def analysis_logs(self,line):
        if(self.re_partition_updation.search(line)):
            ...
            if(search_obj):
                ...
                if(self.common.ota_initiated):
                    ...
                    if(partition in self.common.ota_complete_prints_copy):
                        ...
                        self.common.log.info("Partition [%s] not mentioned in the input xml file but seems like %s got updated"%(partition,partition))
            else:
            ...
            if(search_obj):
                ...
                if(self.common.ota_done):
                    if(not self.common.post_ota_kernal_timestamp):
                        ...
                        self.common.log.info("Post OTA Kernel Timestamp : [%s]"%(self.common.post_ota_kernal_timestamp))
            ...
            if(search_obj):
                ...
                if(self.common.ota_done):
                    if(not self.common.post_ota_tegra_boot_version):
                        ...
                        self.common.log.info("Post OTA TegraBoot Version : [%s]"%(self.common.post_ota_tegra_boot_version))
                    ...
                    if(self.common.base_build_flashed and not self.common.pre_ota_tegra_boot_version):
                        ...
                        self.common.log.info("Pre OTA TegraBoot Version : [%s]"%(self.common.pre_ota_tegra_boot_version))
            ...
            if(search_obj):
                ...
                if(self.common.ota_done):
                    if(not self.common.post_ota_bootloader_version):
                        ...
                        self.common.log.info("Post OTA Bootloader Version : [%s]"%(self.common.post_ota_bootloader_version))
                    ...
                    if(self.common.base_build_flashed and not self.common.pre_ota_bootloader_version):
                        ...
                        self.common.log.info("Pre OTA Bootloader Version : [%s]"%(self.common.pre_ota_bootloader_version))
            ...
            self.common.log.info("This is expected. Will reboot the device and continue test from the current iteration")
...
class OTAtest(Common,BaseTestInterface):
    ...
    def installAppsFromPlaystore(self):
        ...
        if(self.args.install_market_apps):
            ...
            for file_to_push in perforce_jars:
                self.execute_adb_cmd("push %s /data/local/tmp/"%(os.path.join(perforce_jar_location,file_to_push)),timeout=20,raise_exception=False)

            self.log.info("Logging in to Google account [%s] with password [%s]"%(self.args.google_username,self.args.google_password))
            ...
            self.pullLogsFromDevice("/data/qvs_logs", os.path.join(self.gmailLoginLogsDir)) #Pulling gmail login logs from device /data/qvs_logs
            if(returnVal):
                ...
                for appPackage in appPackages:
                    ...
                    (returnVal,output) = DeviceUtils.installAppFromPlaystore(self.args.google_username,self.args.google_password,appPackage)
                    self.logOutputToFile(os.path.join(self.googlePlaystoreLogsDir,"playstore_app_install_stdout.txt"),output)
                    self.pullLogsFromDevice("/data/local/tmp/qvs_playstore_dir", os.path.join(self.googlePlaystoreLogsDir)) #Pulling playstore logs from device
                    if(returnVal):
    ...
    def push_files_to_device(self):
        ...
        """
        if(hasattr(self,'args') and hasattr(self.args,'incremental') and self.args.incremental and not self.ota_done):
            return
        else:
            files_to_push_list = ['utils.sh','busybox']
            self.log.info("\t\tPushing files to device \n\n")
            for file_to_push in files_to_push_list:
                self.execute_adb_cmd("push %s /system/bin/"%(os.path.join(self.utils_path,file_to_push)),timeout=20,raise_exception=False)
                self.execute_adb_cmd("shell chmod 777 /system/bin/%s"%(file_to_push),timeout=20,raise_exception=False)
    ...
    def adb_root_remount(self, exception=False):
        if(hasattr(self,'args') and hasattr(self.args,'incremental') and self.args.incremental and not self.ota_done):
            ...
            try:
                ...
                return_val = self.execute_adb_cmd_getbundle(command="remount",timeout=60,raise_exception=False,large_out=False)
                attempt=1
                while(attempt < 3):
                    disableVerityRegex = ".*adb disable-verity.*disable verity"
                    if((return_val.stdout and re.search(disableVerityRegex,return_val.stdout,re.I)) or (return_val.stderr and re.search(disableVerityRegex,return_val.stderr,re.I))):
    ...
    def parse_cmd_line(self):
        ...
        mutually_exclusive_group.add_argument("-t", "--tnspec", action="store", default="auto", help="board tnspec if flash using flash.sh")
        mutually_exclusive_group.add_argument("-f", "--use_fastboot", action="store_true", help="Use fastboot to flash the device")

        parser.add_argument("-b", "--board_type", action="store", default="unfused", required=True, help="Board variant [fused | unfused]")
        parser.add_argument("-g", "--ota_build_string", action="store", default="TOT", required=True, help="OTA build string that appears in TegraOTA apk [Unique string]. TOT to perform OTA on Top of the tree")
        parser.add_argument("--ota_build_flavor", action="store", help="Provide the OTA build's flavor [user,userdebug]")
        parser.add_argument("-I", "--incremental", help="If set, considers the upgrade as an incremental OTA", action="store_true", default=False)
        parser.add_argument("-k", "--sku", action="store", default="", help="na_wf/na_do/un_do")
        parser.add_argument("-m", "--min_battery_level", action="store", type=int, default=50, help="Minimum battery percentage required for OTA")
        parser.add_argument("-o", "--oldbuild", action="store", default="", required=True, help="Old Build Url")
        parser.add_argument("-T", "--device_boot_time", action="store", type=int, default=300, help="Maximum Boot time in seconds")
        parser.add_argument("--ota_update_partitions", default="NVC|EBT|BCT|RBL", help="Provide partitions that are updated. With refernce to kmsg. Use | as separator.")
        parser.add_argument("--check_ota_partitions", default=None, help="Provide partitions to be pulled and compared to confirm successful OTA.")
        parser.add_argument("--ota_build_path", help="Provide corresponding OTA build path[Required in case of comparing partitions].")
        parser.add_argument("--ota_diff", action="store_true", default=False, help="Use this argument if partition diff needs to be done using ota_diff.py")
        parser.add_argument("-U", "--user_build", action="store_true", default=False, help="Use this argument if the build is user build (usb debugging isn't enabled)")
        parser.add_argument("-v", "--version_check_app_list", default="com.nvidia.tegrazone3|com.frozenbyte.Trine2", help="Provide the package names to check app versions. Use | as separator.")
        parser.add_argument("-w", "--wait_for_min_battery", action="store_true", default=True, help="Waits for minimum battery level before doing an OTA")
        parser.add_argument("--build_utils_path", action="store", default=None, help="Provide the location for mkbootfs/mkbootimg")
        parser.add_argument("--test_name", help="Test name of the current test",default="OTATest")
        parser.add_argument("--encrypt_device", help="If set performs OTA on an encrypted device",default=False, action="store_true")
        parser.add_argument("--encrypt_commandline", help="Commandline for encrypting the device",default="vdc cryptfs enablecrypto inplace default")
        parser.add_argument("--download_install_apps", help="Provide http locations for app paths. Use | as separator.")
        parser.add_argument("--install_market_apps", help="Provide package names of apps to be installed from Google playstore before OTA. Use | as separator.")
        parser.add_argument("--google_username", help="Provide Google user account id. This is required for Google Playstore.")
        parser.add_argument("--google_password", help="Provide Google user account password. This is required for Google Playstore.")

        parser.add_argument("--move_apps_to_sdcard", help="Provide the package names to move to sdcard before OTA. Use | as separator.")
        parser.add_argument("--app_launch_test", help="Provide the package names to perform launch test. Use | as separator.")

        parser.add_argument("--fb_partitions", help="Provide Fastboot partition names and images. Use associate partition names with images. Use | to separate partitions.", default="recovery:recovery.img|boot:boot.img|system:system.img|vendor:vendor.img|userdata:userdata32.img|staging:blob|bmp:bmp.blob")

        self.args = parser.parse_args()

        if((self.args.check_ota_partitions or self.args.ota_diff) and not self.args.ota_build_path):
            raise Exception("Cannot check ota partitions  [--check_ota_partitions] without the corresponding OTA build [--ota_build_path] ")
        if((self.args.install_market_apps) and (not self.args.google_username or not self.args.google_password)):
            raise Exception("[--google_username] or [--google_password] arguments missing. Cannot install Apps from Google Playstore")
    ...
    def rename_csv_file(self):
        ...
        self.execute_cmd("sudo mv -f %s %s"%(os.path.join(self.ota_diff_output_folder,"results.csv"), os.path.join(self.ota_diff_output_folder,"ota_diff_results.csv")))
    ...
    def test_setup(self):
        ...
        result_csv_header = ['Stress Count','Result','Failure Reason','Pre OTA Bootloader Version','Pre OTA Tegraboot Version','Pre OTA Kernel timestamp','OTA Build String','Post OTA Bootloader Version','Post OTA Tegraboot Version','Post OTA Kernel timestamp','Data Persistance Check','Modem firmware check','wifi check','Modem Data Check']
        ...
        if(self.args.user_build):
            self.log.warning("*** ALERT ***   User Production images not supported in OTA stress. No way to enable adb. Test might fail")
    ...
    def performNonMarketAppLaunchTest(self):
        if(self.args.app_launch_test):
            ...
            for packageName in app_launch_test_results.keys():
                app_launch_test_result_string="%s%s %s|"%(app_launch_test_result_string, packageName, app_launch_test_results[packageName])
            ...
            if(not self.ota_done):
                ...
                for packageName in app_launch_test_results.keys():
                    if("fail" in app_launch_test_results[packageName].lower()):
                        self.add_to_marketAppsTestResult(packageName,"Failure Reason",app_launch_test_results[packageName])
                ...
                for packageName in app_launch_test_results.keys():
                    self.add_to_marketAppsTestResult(packageName,"Post OTA Package Launch Status",app_launch_test_results[packageName])
    ...
    def data_persistence_setup(self):
        ...
        if(self.execute_adb_cmd("push %s /sdcard/"%(os.path.join(self.working_dir,"data_persistance")),timeout=20)==0):
    ...
    def connect_wifi(self):
        ...
        return_val = self.execute_adb_cmd_getbundle(command="shell netcfg",timeout=60,raise_exception=False,large_out=False)
    ...
    def pre_ota_setup(self):
        ...
        if(self.execute_cmd("echo %s > base_kernel_timestamp.txt"%(self.pre_ota_kernal_timestamp),timeout=20)==0):
            if(self.execute_adb_cmd("push base_kernel_timestamp.txt /sdcard/",timeout=20)==0):
                ...
                self.log.warning("Failed to push Base build kernel timestamp to device [/sdcard/base_kernel_timestamp.txt]")
                return False
        else:
    ...
    def init_ota_test_params(self,stress_count):
        ...
        self.result_dict                    = {'Stress Count':-1,'Result':"Untested",'Failure Reason':"Unknown",'Pre OTA Bootloader Version':"Unknown",'Pre OTA Tegraboot Version':"Unknown",'Pre OTA Kernel timestamp':"Unknown",'OTA Build String':"Unknown",'Post OTA Bootloader Version':"Unknown",'Post OTA Tegraboot Version':"Unknown",'Post OTA Kernel timestamp':"Unknown",'Data Persistance Check':"Untested",'Modem firmware check':"Untested",'wifi check':"Untested",'Modem Data Check':"Untested"}
        ...
        if(self.args.install_market_apps or self.args.app_launch_test):
            ...
            if(self.args.install_market_apps):
                ...
                market_apps_result_csv_filePath = os.path.join(self.test_iteration_log_dir,self.market_apps_result_csv_filename)
                market_apps_result_csv_header = ['Package Name','Result','Failure Reason','Pre OTA Package Status','Pre OTA Package Version','Pre OTA Package Launch Status','Post OTA Package Status','Post OTA Package Version','Post OTA Package Launch Status']
        ...
        if(DeviceUtils.wait_for_boot_complete(boot_wait_time)):
            ...
            if DeviceUtils.is_device_offline():
                ...
                for command in command_list:
                    if not self.uart.send_command(command="%s && echo __RETURN_CODE_$?_"%(command),expected="RETURN_CODE_0",time_out=5):
    ...
    def flash_and_wait_for_boot(self,stress_count):
        ...
        if(not self.flash_base_build(flash_log)):
            ...
            for attempt in range(1,4):
                device_bootup_status = self.wait_for_boot_completion(1200) if(attempt == 1) else self.reset_and_wait_for_boot_completion(1200)
    ...
    def writeMarketAppsResult(self):
        ...
        if(self.args.install_market_apps):
            for packageName in self.args.install_market_apps.split("|"):
                if(self.marketAppsResultDict[packageName]['Failure Reason'] != None and self.marketAppsResultDict[packageName]['Failure Reason'].lower() not in ("unknown","untested")):
                    marketAppsResultStatus = "Failed"
                if(self.marketAppsResultDict[packageName]['Failure Reason'] == None or self.marketAppsResultDict[packageName]['Failure Reason'].lower() in ("unknown","untested")):
            ...
            with open(recovery_log,"r") as recovery_lf:
                for line in recovery_lf:
                    ...
                    if(search_obj):
                        ...
                        self.log.info("Found OTA success print in recovery log. Maybe OTA was done but UART logs for partition updation wasn't found...")
    ...
    def encrypt_device(self):
        ...
        if(self.execute_adb_cmd('shell "%s"'%(self.args.encrypt_commandline),timeout=600,raise_exception=False)==0):
    ...
    def get_kernel_timestamp(self):
        if(not self.pre_ota_kernal_timestamp):
            ...
            self.log.info("Looks like I'm not getting kernel timestamp from UART logs. Will try with [%s]"%(cat_command))
    ...
    def updateOTATestProgress(self, iteration=0, message=None):
        if(hasattr(self.args,"job_id") and self.args.job_id):
            try:
                if (os.path.exists(os.path.abspath(os.path.join(os.path.dirname(__file__),"..","..","..","..","Scripts")))):
                ...
                additionalInfo="%s [%d/%d] [Q: %.2f] [P: %d] [F: %d]"%(additionalInfo, iteration,self.args.stresscount,currentQScore,self.passedIterations,self.failedIterations)
        ...
        for stress_count in range(1,self.args.stresscount+1):
            ...
            if(self.flash_and_wait_for_boot(stress_count)):
                ...
                while(retry <= ota_retries):
                    ...
                    self.test_iteration_attempt_log_dir = os.path.join(self.test_iteration_log_dir,"ota_attempt_%d"%(attempt))
                    ...
                    if(not DeviceUtils.is_device_on()):
                        ...
                        self.add_to_result('Failure Reason',"Either device isnt ON or usb debugging not enabled. Skipping test")
                        #break;
                    else:
                        self.connect_wifi()
                    ...
                    if(self.ota_done):    
                        self.updateOTATestProgress(stress_count,"Successfully OTAed. Waiting for post OTA test results")
                        self.reset_and_wait_for_boot_completion()
    ...
    def flash_build(self,flash_log = None):
        ...
        if(self.args.use_fastboot):
            ...
            self.get_tegraflash_command(flash_log) #TO KNOW THE TEGRAFLASH FLASHING COMMAND WHICH CAN BE USED WHILE READING PARTITIONS
    ...
    def run_ota_diff(self):
        ...
        self.execute_cmd('echo "=============================NEW_RUN_STARTING=============================" >> %s' % self.ota_diff_output_file)
        return_val = self.execute_cmd_getbundle("sudo python otadiff_core.py -d %s >> %s"%(self.ota_diff_output_folder,self.ota_diff_output_file),timeout=3600,large_out=True)
        if return_val.error_code:
    ...
    def post_flash_settings(self):
        ...
        self.disable_packages(["com.google.android.setupwizard","com.google.android.tungsten.setupwraith","com.nvidia.shield.welcome","com.nvidia.calibnotifier"])
    ...
    def start_json_RPC_server(self,port=9008):
        ...
        if(return_val.error_code==0):
            ...
            return_val = self.execute_adb_cmd_getbundle("forward --list | grep %d"%(port),timeout=60,raise_exception=False,large_out=False)
            if(return_val.error_code==0):
            ...
            if(os.path.exists(uiautomatorLibsPath)):
                ...
                for library in files:
                    if(library.endswith(".jar")):
                        ...
                        if(self.execute_adb_cmd("push %s /data/local/tmp/"%(libraryAbsolutePath),timeout=60,raise_exception=False,verbose=True)==0):
                        ...
                        if(self.execute_adb_cmd("install -r %s "%(libraryAbsolutePath),timeout=60,raise_exception=False,verbose=True)==0):
    ...
    def ui_auto_launch_nvidia_OTA_app(self,ui_device):
        if(DeviceUtils.is_package_installed(self.ota_app_package_name)):
            ...
            if(self.args.incremental):
                ...
                self.log.info("Launching Nvidia OTA app [Version : %s]... ACTION [%s]"%(ota_app_version_name,launcherActivityAction))
    ...
    def ui_auto_choose_ota_branch(self,ui_device):
        ...
        if (ui_device(text=self.ota_app_ui_elements[self.ota_app_version]["text_branch"]).wait.exists(timeout=60000)):
            ...
            if(ui_device(text=self.args.branch).wait.exists(timeout=10000)):
                ...
                ui_device.screenshot(os.path.join(self.test_iteration_attempt_log_dir,'no_branch_%s.png'%(self.args.branch)))
            ...
            ui_device.screenshot(os.path.join(self.test_iteration_attempt_log_dir,'no_choose_branch_%s.png'%(self.args.branch)))
    ...
    def ui_auto_select_ota_build(self,ui_device):
        ...
        if self.args.ota_build_string.lower() == "tot":
            ui_device(className='android.widget.RelativeLayout',index=self.ota_app_ui_elements[self.ota_app_version]["tot_index"]).click()
            return True
        else:
            if ui_device(scrollable=True).scroll.to(textContains=self.args.ota_build_string):
                ...
                ui_device.screenshot(os.path.join(self.test_iteration_attempt_log_dir,'no_build_%s.png'%(self.args.ota_build_string)))
    ...
    def new_ui_auto_select_ota_build(self,ui_device):
        self.log.info("Selecting OTA build...")
        if self.args.ota_build_string.lower() == "tot":
            build_selector = ui_device(className='android.widget.RelativeLayout',index=self.ota_app_ui_elements[self.ota_app_version]["tot_index"])
            ...
            if(self.args.ota_build_flavor):
                self.log.info("Selecting Build [%s] | Flavor [(%s)]"%(self.args.ota_build_string,self.args.ota_build_flavor))
                ...
                while(True):
                    ...
                    retry = 0
                    while(retry <=3):
                        build_selector = ui_device(textContains=self.args.ota_build_string).sibling(textContains=flavorText)
                        if build_selector.exists:
                            self.ota_build_name = build_selector.sibling(textContains=self.args.ota_build_string).info['text']
                            self.log.info("Clicking OTA build [%s]\n"%(self.ota_build_name))
                            self.add_to_result('OTA Build String',self.ota_build_name)
                            build_selector.click()
                            return True
                    ...
                    if(not scrollResult):
                        ...
                        if(i > 1):
                            break
                    self.log.warning("OTA build string [%s] Flavor [%s] not found"%(self.args.ota_build_string,self.args.ota_build_flavor))
            else:
                if ui_device(scrollable=True).scroll.to(textContains=self.args.ota_build_string):
                    self.ota_build_name = ui_device(textContains=self.args.ota_build_string).info['text']
                    self.log.info("Clicking OTA build [%s]\n"%(self.ota_build_name))
    ...
    def ui_auto_download_ota_build(self,ui_device):
        ...
        uiDownloadButton = self.uiGetUIElementByText(ui_device(className='android.widget.Button'),"Download",timeToWait=60000)
        if(uiDownloadButton):
            ...
            return False
        if(ui_device(resourceId = self.ota_app_ui_elements[self.ota_app_version]["resourceid_download"]).wait.exists(timeout=60000)):
            ...
            if(ui_device(textContains=self.ota_app_ui_elements[self.ota_app_version]["text_download_finished"]).wait.exists(timeout=3600000)):
    ...
    def ui_auto_flash_ota_build(self,ui_device,retry):
        ...
        uiFlashButton = self.uiGetUIElementByText(ui_device(className='android.widget.Button'),"Flash",timeToWait=30000)
        if(uiFlashButton):
            uiFlashButton.click()
            if(self.ota_app_version == "1"):
                uiInstallButton = self.uiGetUIElementByText(ui_device(className='android.widget.Button'),"Install",timeToWait=30000)
                if(uiInstallButton):
        ...
        try:
            if(d):
                if(d(textContains='Wi-Fi unstable, due to default MAC').wait.exists(timeout=5000)):
                    self.log.warning("Looks like device wifi is unstable. Device has MAC issues. Test might fail due to this...")
    ...
    def battery_check(self):
        ...
        if(battery_level != -1):
            if(battery_level < self.args.min_battery_level):
                self.log.warning("Battery level less than minimum battery level [%d] required for OTA..."%(self.args.min_battery_level))
                ...
                try:
                    if(not DeviceUtils.wait_for_battery_level(self.args.min_battery_level)):
                        ...
                        self.add_to_result('Failure Reason',"Battery level %d < Minimum battery level required for OTA %d"%(battery_level,self.args.min_battery_level))
                except Exception:
                    self.log.warning("Exception occured while waiting for battery charging...")
                    self.log.warning(traceback.format_exc())
        else:
            self.log.warning("Failed to get the Battery level. OTA can fail due to battery level < %d"%self.args.min_battery_level)
    ...
    def perform_ota_update(self,retry):
        ...
        if(not DeviceUtils.unlock_device()):
            self.log.warning("Failed to unlock the device. Oh God ! I suspect OTA would fail as device isn't unlocked...")
        ...
        if(DeviceUtils.is_device_on()):
            ...
            self.add_to_result('Failure Reason',"Looks like device is not on...I suspect the error happened while flashing the OTA build")
    ...
    def parse_ftp_result_file(self,data):
        ...
        if(not self.post_ota_kernal_timestamp):
            self.log.warning("Oops ! I don't have the post OTA kernal timestamp. Looks like uart logs aren't coming up completely after post OTA")
    ...
    def pull_partitions(self,partition_dict,location):
        if(not self.tegraflash_command):
            self.log.info("Couldn't figure out the tegraflash command used for flashing. Will try using the default command")
        self.log.info("Putting the device to recovery mode...")
        PM342.force_recovery(self.args.board)
        ...
        for partition in cmp_partitions:
            ...
            if(len(partition) > 0):
                ...
                partition_file = self.android_build_utils.read_partition(partition,target_file,tegra_flash_commandline=self.tegraflash_command)
                if(partition_file):
        ...
        for partition in cmp_partitions:
            if(partition and self.orig_partition_list[partition] and self.ota_partition_list[partition]):
                if(FileHelper.diff(self.orig_partition_list[partition],self.ota_partition_list[partition])):
                    self.log.info("Partition %s Matches [%s] [%s]"%(partition,self.orig_partition_list[partition],self.ota_partition_list[partition]))
                else:
                    self.log.warning("Partition %s differs [%s] [%s]"%(partition,self.orig_partition_list[partition],self.ota_partition_list[partition]))
